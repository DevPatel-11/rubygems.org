...
class RecorderTest < ActiveSupport::TestCase
  ...
  test "records a gem push event when record is a Pusher" do
    ...
    assert_nothing_raised { @controller.send(:record_transparency_log_event, "gem_push", pusher) }
  end

  test "records a gem deletion event when record is a Deletion" do
    user = create(:user)
    api_key = create(:api_key, owner: user)
    rubygem = create(:rubygem)
    version = create(:version, rubygem:)
    deletion = Deletion.allocate

    Deletion.any_instance.stubs(:version).returns(version)
    ...
    assert_nothing_raised { @controller.send(:record_transparency_log_event, "gem_yank", deletion) }
  end
  ...
  test "records an ownership change event when record is a User" do
    ...
    assert_nothing_raised { @controller.send(:record_transparency_log_event, "owner_add", owner) }
  end

  test "does not record an event for an unsupported record type" do
    TransparencyLog::Recorder.expects(:new).never

    assert_nothing_raised { @controller.send(:record_transparency_log_event, "unknown_event", Object.new) }
  end
  ...
  test "reports errors without raising them" do
    ...
    TransparencyLog::Recorder.expects(:new).raises(error)
